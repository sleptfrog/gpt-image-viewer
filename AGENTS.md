...
- 画像ページ由来のURL辞書件数・チャット紐づき件数・最終取り込み時刻の表示
- ChatGPTの「画像」ページを人間操作相当の速度で読み進めるスクロール補助
...
- 選択画像のZIP保存
- 画像URL辞書内の取得済み画像をユーザーが選択したローカルフォルダへチャット名またはチャットIDごとに一括保存
- 単体画像保存
...
画像URLがまだ辞書にない項目は、UI上では「画像未取得」と表示します。この場合はChatGPTの「画像」ページを開き、対象画像が読み込まれるまでスクロールすることでURL辞書に取り込めます。辞書内の取得済み画像は、ユーザーが選択したローカルフォルダへ、チャット名またはチャットIDごとに一括保存できます。
...
`src/metadata/parse-recent-image-gen.ts` はChatGPTの「画像」ページで読み込まれるrecent image responseから画像URL辞書用レコードを抽出します。URLに加えて、conversation ID、message ID、asset pointer、generation ID、画像サイズ、caption/prompt/titleなど、後続の一括保存に使える最小化済み情報も扱います。
...
- ChatGPTの「画像」ページを開かずに全画像URLを直接取得する機能は未実装です。画像ページ上ではスクロール補助でChatGPT自身の読み込みを進められます。
